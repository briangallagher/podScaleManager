...
var cpuAllocationMilliCores = process.env.CPU_MILLICORE_LIMIT;
var memoryAllocationMB = process.env.MEMORY_LIMIT;
var blocked = false;
var stepCount;
...
// We can only scale down if all criteria are < 0
// If any item is =0 or > 0 it means we are within our treshold
// or require more resources
function eligibleForScaleDown(array) {
...
exports.go = function (data) {
  ...
  getModel(function (model) {
    ...
    // reset to 3 on every run
    stepCount = 3;

    if (model.cpuTresholdPercent && currentCPUAllPods != -1) {
      stepCount++;
    ...
    if (model.memoryTresholdPercent && currentMemoryAllPods != -1) {
      stepCount++;
    ...
    if (model.upperResponseTimeTresholdSeconds && currentResponseTime != -1) {
      stepCount++;
    ...
    if (max > 0) {
      ...
      console.log('Requesting new Total Pods: ' + total);

      spinUp(total);

    } else if (eligibleForScaleDown(array)) {
      total = currentPods - 1; // At the moment we only spin down by 1
...
function checkCPUStatus(currentCPUAllPods, currentNumPods, cpuThresholdMilliCores) {
  ...
  console.log('Current treshold in MilliCores %s', cpuThresholdMilliCores);
  console.log('Current total treshold in MilliCores %s', totalThreshold);
  console.log('Current cpu usage across all pods in MilliCores %s \n', currentCPUAllPods);

  if (currentCPUAllPods > totalThreshold) {
    diff = currentCPUAllPods - totalThreshold;
    extraPods = Math.ceil(diff / cpuThresholdMilliCores);

    console.log('Current total CPU usage is greater tn total threshold, extra %s pods required to meet CPU demand', extraPods);
...
function checkMemoryStatus(currentMemoryAllPods, currentNumPods, memoryThresholdMBs) {
  ...
  console.log('Current number of pods %s', currentNumPods);
  console.log('Current treshold in MBs %s', memoryThresholdMBs);
  console.log('Current total treshold in MBs  %s', totalThreshold);
  console.log('Current memory usage across all pods in MBs %s \n', currentMemoryAllPods);
...
function checkIOStatus(currentResponseTime, currentPods, upperTreshold, lowerTreshold) {
  ...
  console.log('Current upper response time treshold in secs %s', upperTreshold);
  console.log('Current lower response time treshold in secs %s', lowerTreshold);
  console.log('Current average response time across all pods in secs %s \n', currentResponseTime);
